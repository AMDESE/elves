...
# @Module Name: sysfs-qos-llc-test.py
# @Description: Check resource control support, mount, callocate half cache and validate
...
testDir = os.path.join(RESCTRL_PATH, "test")
...
class Resctrl(Test):
    ...
    # and allocate half cache bandwidth to core complex 0
    def test_pqos_resctrl_allocate(self):
        ...
        assert exitstatus == 0
        (exitstatus, stdout, _) = CommonLib.Run(["echo 'L3:0=0xff' > {0}/schemata".format(testDir)])
        assert exitstatus == 0
