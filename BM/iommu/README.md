...
devices. In addition, IOMMU does support other features like SVA, ATS etc. For more
details please refer below IOMMU specification

> - https://www.amd.com/content/dam/amd/en/documents/processor-tech-docs/specifications/48882_IOMMU.pdf

> Some kernel command line options available for IOMMU
...
> IOMMU should be tested for different boot mode combination. Some are listed below 
...
> 1.  **Change IOMMU domain type test** 
...
> - identity: All the DMA transactions from the device in this group are not translated
> by the IOMMU.
...
> Running the test : ***$ avocado run avocado-misc-tests/io/iommu/iommu\_tests.py -p
> pci_devices="{full_pci_address}" -p dmesg_grep="iommu|AMD-Vi" -p count=4 -\-max-parallel-tasks=1*** 

> **Note:** Running this test needs at least one pci device input 'pci\_devices'.
> eg. ***$ avocado run avocado-misc-tests/io/iommu/iommu\_tests.py -p
> pci_devices="0000:01:00.0" -p dmesg_grep="iommu|AMD-Vi" -p count=4 -\-max-parallel-tasks=1***
...
> 2.  **IOMMU v2 page table level validation test** 
...
> Running the test : ***$ avocado run avocado-misc-tests/io/iommu/amd/iommu\_v2pgmode\_test.py *** 
